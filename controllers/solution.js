...
const getByCode = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json(
        'An error occurred while retrieving that game, but you can still use the locally stored solutions with route /local/0-999.'
      );
  }
};
...
const addToClicked = async (req, res) => {
  ...
  try {
    ...
    if (!game.clicked) {
      ...
      response = await gameCollection.updateOne(
        { code: code },
        { $set: { clicked: [clickedString] } }
      );
    } else {
      // 'clicked' field exists, push the clickedString to the array
      response = await gameCollection.updateOne(
        { code: code },
        { $push: { clicked: clickedString } }
      );
    }
  } catch (error) {
    console.error(error);
  ...
  if (response.acknowledged) {
    ...
    res
      .status(500)
      .json(response.error || 'could not update the clicked cards.');
  }
};
